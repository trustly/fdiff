...
CREATE OR REPLACE VIEW view_functions AS
     SELECT
        ...
        n.nspname AS schema,
        p.proname AS name,
        pg_get_function_result(p.oid) AS resultdatatype,
        pg_get_function_arguments(p.oid) AS argumentdatatypes,
        CASE
        ...
        COALESCE(p.proconfig,ARRAY['']::text[]) AS configurationparameters
   FROM pg_proc p
